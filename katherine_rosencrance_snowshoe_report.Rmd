...
**Figure 3: Hind foot length (mm) versus weight (g) in juvenile snowshoe hares.** Hind foot length is measured in mm and weight is measured in grams. Each point represents a hare observed at the specific weight and hind foot length. The black line is the line of best fit in the model.β1 = 49.69 g mm-1, p < 0.001, R2 = 0.76, Pearson’s r = 0.87)

### Analysis

The relationship between hare hind foot length and weight, upon observation, is positively correlated. After running a linear regression, one can see that the p-value returned was `r juvenile_lm_glance$p.value`. This is a very low p-value so one might conclude that the likelihood of getting two samples this different if samples were taken from populations with the same mean is very small. In addition, the p-value rendered when comparing the hind foot and weight via a Pearson's correlation was also low. The r squared value of `r round(juvenile_lm_glance$r.squared, 2)` however, represents the fact that only `r (round(juvenile_lm_glance$r.squared,2))*100`% of the variation in weight is attributed to hind foot length. This means that other factors like sex and age should be taken into account when seeing what affects weight.
